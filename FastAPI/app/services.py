...
def insert_vectors(mivus_collection: Collection, ids: List[int], vectors: np.ndarray):
    # Milvus에 데이터 삽입
...
async def load_and_vectorize_animal_data():
    ...
    insert_vectors(animal_collection, ids, reduced_vectors)
...
async def load_and_vectorize_group_data():
    ...
    insert_vectors(group_collection, ids, reduced_vectors)
...
async def update_new_animals(animal_index, animal_matrix):
    ...
    insert_vectors(animal_collection, new_ids, reduced_new_vectors)
...
async def update_new_groups(group_index, group_matrix):
    ...
    insert_vectors(group_collection, new_ids, reduced_new_vectors)

    current_length = len(group_matrix)
    group_matrix = np.vstack([group_matrix, reduced_new_vectors])
