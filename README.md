...
> 한국에서는 반려동물 입양 의향이 증가하고 있지만, **유기동물 발생률은 여전히 높고**, **보호소를 통한 입양률은 낮은** 상황입니다. 주요 문제로 **낮은 보호소 입양률**과 **부족한 보호 인프라**가 대두되고 있습니다. **ForPaw**는 이러한 문제를 해결하기 위해 유기동물 입양을 지원하고 보호소 봉사활동을 촉진하는 플랫폼입니다.</br>
</br>
...
이러한 이유로, 우리는 이 두 가지 문제를 **웹 서비스**로 해결하고자 합니다. 법적으로 보호되지 못하는 부분을 서비스에 담아 사각지대를 메우고, 개인이 해결하기 어려운 문제들을 하나의 플랫폼에서 편리하게 해결할 수 있도록 계획하였습니다.</br>
</br>
...
| 이름   | 역할 | 내용 |
|:----:|:----:|------|
| 이한홍  | BE (리드) | <ul><li>REST API 및 DB 설계</li><li>FastAPI와 SpringBoot를 활용한 주요 API 개발 (API #1 ~ #102)</li><li>Figma를 활용한 UI/UX 디자인 (관리자 페이지 등)</li></ul> |
