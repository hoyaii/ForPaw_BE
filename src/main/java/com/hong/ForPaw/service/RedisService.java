...
public class RedisService {
    ...
    // 저장된 데이터와 일치여부 비교
    public boolean validateData(String type, String id, String token){
        String storedToken = redisTemplate.opsForValue().get(buildKey(type, id));

        return storedToken.equals(token);
    }

    // 데이터 삭제
    public void removeData(String type, String id) {
        redisTemplate.delete(buildKey(type, id));
    }

    // 데이터 반환 - Long 반환
    public Long getDataInLong(String type, String id){
        String num = redisTemplate.opsForValue().get(buildKey(type, id));
